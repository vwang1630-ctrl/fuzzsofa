        ...
        <div className="space-y-0">
          {journalArticles.map((article, i) => (
            <Link
              ...
              className="group block border-t border-[#222] py-8 first:border-t-0 hover:border-[#E8B4B8]/30 transition-colors duration-300"
            >
              <div className="flex flex-col md:flex-row md:items-baseline gap-2 md:gap-8">
                <p className="text-xs text-[#6B6B6B] tracking-[0.1em] uppercase shrink-0 w-24">
                  {article.category}
                </p>
                ...
                <p className="text-xs text-[#6B6B6B] shrink-0">{article.datePublished}</p>
              </div>
            </Link>
          ))}
        </div>
