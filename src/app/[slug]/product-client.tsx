                ...
                <div className="max-w-[1200px] mx-auto px-4 md:px-8 pt-4 lg:pt-8 pb-8 lg:pb-12">
                    ...
                    <div className="lg:hidden -mx-4 md:-mx-8 relative">
                        ...
                        {galleryImages.length > 1 && <>
                            <button onClick={() => { const prev = (activeImage - 1 + galleryImages.length) % galleryImages.length; setActiveImage(prev); const el = mobileGalleryRef.current?.children[prev] as HTMLElement; el?.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' }); }} className="absolute left-3 top-1/2 -translate-y-1/2 z-20 transition-opacity duration-200 active:opacity-60" aria-label="Previous">
                                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#F5F0EB" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
                            </button>
                            <button onClick={() => { const next = (activeImage + 1) % galleryImages.length; setActiveImage(next); const el = mobileGalleryRef.current?.children[next] as HTMLElement; el?.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' }); }} className="absolute right-3 top-1/2 -translate-y-1/2 z-20 transition-opacity duration-200 active:opacity-60" aria-label="Next">
                                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#F5F0EB" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg>
                            </button>
                        </>}
