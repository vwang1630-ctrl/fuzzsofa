...
export function Header() {
    ...
    return (
        <header
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${headerSolid ? "bg-[#0A0A0A]/95 backdrop-blur-[8px] border-b border-[#1A1A1A]" : "bg-transparent border-b-0"}`}
            dir={isRtl ? "rtl" : "ltr"}>
            <div
                className="max-w-[1200px] mx-auto px-6 h-[60px] flex items-center justify-between">
                {}
                <Link href="/" className="font-serif text-xl tracking-[0.08em] text-[#F5F0EB]">FUZZ SOFA
                            </Link>
                {}
                <nav
                    className="hidden sm:flex items-center gap-6 text-xs font-light tracking-[0.1em] uppercase">
                    <Link
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div ref={langRef} className="relative">
                        ...
                        {langOpen && <div
                            className={`absolute top-full mt-2 right-0 border rounded-[4px] py-1 min-w-[160px] max-h-[300px] overflow-y-auto z-50 ${isProductPage ? "bg-black/90 backdrop-blur-md border-white/10" : "bg-[#111111] border-[#1A1A1A]"}`}>
                            {locales.map((loc: Locale) => <button
                                key={loc}
                                onClick={() => {
                                    setLocale(loc);
                                    setLangOpen(false);
                                }}
                                className={`w-full text-left px-4 py-2 text-xs tracking-[0.05em] hover:bg-[#1A1A1A] transition-colors ${loc === locale ? "text-[#E8B4B8]" : "text-[#8A8580]"}`}>
                                {localeNames[loc]}
                            </button>)}
                        </div>}
                    </div>
                    ...
                    <div ref={userMenuRef} className="relative hidden sm:block">
                        ...
                        {userMenuOpen && user && <div
                            className="absolute top-full mt-2 right-0 bg-[#111111] border border-[#1A1A1A] rounded-[4px] py-1 min-w-[180px] z-50">
                    ...
                    <Link
                        ...
                        {totalItems > 0 && <span
                            className="absolute -top-2 -right-2 w-5 h-5 bg-[#E8B4B8] text-[#0A0A0A] text-[10px] rounded-full flex items-center justify-center font-medium">
                            {totalItems}
                        </span>}
                    </Link>
                    {}
                    <button
                        ...
                        className={`sm:hidden transition-colors ${headerSolid ? "text-[#8A8580] hover:text-[#E8B4B8]" : "text-white/50 hover:text-white"}`}
            ...
            {}
            {mobileOpen && <nav
                className="sm:hidden bg-[#0A0A0A] border-t border-[#1A1A1A] px-6 py-4 flex flex-col gap-4 text-xs font-light tracking-[0.1em] uppercase">
                <Link
                    href="/animal-sofa-collection"
                    onClick={() => setMobileOpen(false)}
                    className="text-[#8A8580] hover:text-[#E8B4B8] transition-colors">{t("collection")}</Link>
                <Link
                    href="/magazine"
                    onClick={() => setMobileOpen(false)}
                ...
                <Link
                    href="/about"
                    onClick={() => setMobileOpen(false)}
                    className="text-[#8A8580] hover:text-[#E8B4B8] transition-colors">{t("about")}</Link>
                <Link
                    href="/contact"
                    onClick={() => setMobileOpen(false)}
                    className="text-[#8A8580] hover:text-[#E8B4B8] transition-colors">{t("contact")}</Link>
                {user ? <Link
                    href="/account"
                    onClick={() => setMobileOpen(false)}
                    className="text-[#8A8580] hover:text-[#E8B4B8] transition-colors">{t("myAccount" as TranslationKeys)}</Link> : <Link
                    href="/login"
                    onClick={() => setMobileOpen(false)}
                    className="text-[#8A8580] hover:text-[#E8B4B8] transition-colors">{t("signIn" as TranslationKeys)}</Link>}
                <div className="flex gap-3 pt-2 border-t border-[#1A1A1A] items-center">
                    <button
                        ...
                        className="text-[#8A8580] hover:text-[#E8B4B8] transition-colors text-xs tracking-[0.1em] uppercase">🌐 {localeNames[otherLocale]}</button>
